...
$$ Min \; -\sum_{i,j} P( a_i \text{ et } t_j  \text{ et }  r) \ln(P( a_i \text{ et } t_j  \text{ et }  r)) $$
sous les contraintes
...
# POV le 5 octobre : il reste à calculer Eta_rev en tenant compte de P(ai) à partir du tableau de contingence
...
if(( (M_a[i]%in% modalite) | (M_t[j] %in% modalite) ) & ( (R[k]>=rmin) & (R[k]<=rmax) )){
  ligne[l]=1
  }
  else {
  }
...
n_cont_rev=10*(N_mod[1]+N_mod[2])# POV ; le nombre de décile est en dur !! A améliorer !
Eta_rev=matrix(rep(0.1, n_cont_rev), nrow=n_cont_rev, ncol=1) 
l=1;
for(i in 1:nrow(D_at)){
  modalite=D_at$M[i]
  p_modalite=sum(C[( (C$a %in% modalite) | (C$t %in% modalite) ),]$p)
 for( k in 1:10 ){
  rmin=D_at[1+k]
  rmax=D_at[1+k+1]
  if( (i==1) & (k==1) ){
...
Sortie=minxent.multiple(q=p_init,G=Mom[I,],eta=Eta[I],lambda=rep(0, length(I)-1))  # Quitting from lines 528-624 (methodologie_revenue_menage.Rmd) 
