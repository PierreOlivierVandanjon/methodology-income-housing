...
```
# Mise en oeuvre de la méthodologie heuristique par minimisation de l'entropie croisée
...
# Mise en oeuvre de la méthodologie  par minimisation de l'entropie
...
Dans notre cas : 4x3 modalités croisés x (9x7 déciles+2 (le rmin et le rmax))=780 degrés de liberté dans ce cas simple !

Contraintes
  * 12 contraintes sur les probabibilités jointes
  * 10*7 contraintes sur les revenus
...
```
La construction des contraintes est fastidieuses et dépend de l'ordre de classement des probabilitéés.
...
condmax=1e6; # POV pourquoi pas erreur À 200 et à 50 quand la population augmente, algorithme pas étudié pour les grandes degrés de liberté, plus le conditionnement est important, mieux on colle aux marginaux
    lambda0 <- log(sum(q * exp(-lambda * G[-1, ])))
    repeat {
        lambda_old <- lambda
...
############# FIN  Partie du code permettant de lancer l'optimisation SQP  
 ...
 tryCatch (
        ...
        error = function(e){
            message('Erreur sur pov_minxent.multiple (maximisation de l'entropie) : utilisation de la   ')
            sqp=nloptr(x0=p_adm, eval_f=ENtropie, eval_grad_f = GEntropie, lb = lb, ub = ub, eval_g_ineq = NULL, eval_jac_g_ineq = NULL, eval_g_eq = EGalite, eval_jac_g_eq = GEgalite, opts=list("algorithm"= "NLOPT_LD_SLSQP", "xtol_rel"=1.0e-2))
            p_out=sqp$solution
            print(e)
        },
        warning = function(w){
